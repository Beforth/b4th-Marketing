...
from .models import Campaign, Lead, EmailTemplate, CampaignMetric, LeadActivity, Customer, CustomerLocation, Region, Visit, Expense, Exhibition, Quotation, PurchaseOrder, WorkOrder, Manufacturing, Dispatch, URS, GADrawing, TechnicalDiscussion, Negotiation, QCTracking, ProductionPlan, PackingDetails, DispatchChecklist
...
def visit_list(request):
    ...
    # Get all visits
    visits = Visit.objects.select_related('customer', 'assigned_to').all()
...
def negotiation_create(request):
    ...
    if request.method == 'POST':
        ...
        customer_counter = request.POST.get('customer_counter')
...
def negotiation_list(request):
    ...
    negotiations = Negotiation.objects.select_related('quotation', 'conducted_by').all().order_by('-negotiation_date')
